...
| 原则 | **不是首次默认推荐**；仅用户明确要轻度/Demo，或无 Key 选了免费退路时出示；**有啥推啥** |
...
**禁止**把本卡当作「做个视频」的首次推荐。默认路径是电商宣传片。

仅当满足**全部**：

1. 安装闭环 + `verify_ready`（或等价 ready）已过  
2. **尚无**已锁定简报（表 1–3 / `video_plan`）  
3. 用户**明确**说「讲解 / Demo / 轻度短片 / 先试流程」，**或**无视频 Key 且用户选了「这次走免费轻度」

若用户同时要商品宣传片，走电商路径，**不出**本卡。

确认后必须 `backlot open` 并发完整 URL（与商品片同一话术：`你可以查看该网址了解详细信息：{URL}`）。

非首次（已有完成简报或成片）：可一句带过「也可再做短 demo」，默认仍按电商/已有主题走。
...
| 用户选择 | 行为 |
|----------|------|
| 确认 Demo 卡 | 写入轻度锁定字段 → 开板给网址 → **04**；**不**再强制走完整表 1→2→3 |
| 用户未点名轻度 | **不出本卡**；走 03 默认电商入口 |
| Demo 出片后再做商品片 | 新项目：走电商入口 |
