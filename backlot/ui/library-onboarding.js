...
export const REVIEW_MODE_IDS = ["minimal", "normal", "pro"];
...
const REVIEW_MODE_META = {
  minimal: {
    id: "minimal",
    label_zh: "极简",
    summary_zh: "三停：方案、素材、交付。素材通过后直接生成正式段，不做独立试片；失败、缺 Key、超预算才回聊天。",
...
export function normalizeReviewMode(mode) {
  return REVIEW_MODE_IDS.includes(mode) ? mode : DEFAULT_REVIEW_MODE;
}
...
export function getReviewModeRoute(mode) {
...
export function buildCreateProductVideoPrompt(mode) {
  const id = normalizeReviewMode(mode);
...
export function readStoredReviewMode(storage) {
  try {
...
export function writeStoredReviewMode(storage, mode) {
  const id = normalizeReviewMode(mode);
  try {
