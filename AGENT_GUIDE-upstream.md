# OpenMontage — 上游 Pipeline 附录

**日常「做个视频」默认不要读本文件。**

本文件从原 AGENT_GUIDE.md 迁出，供以下情况使用：

- 用户明确点名上游 pipeline（如 `animated-explainer`、`cinematic`、`character-animation`）
- 需要 atelier / 定制合成，或 BootStrap 七 Skill 盖不住简报
- 调试 `product/pipelines/`、stage director、registry preflight

日常契约与双通道（网站 / 聊天）见 [`AGENT_GUIDE.md`](AGENT_GUIDE.md)；门口路由见 [`AGENTS.md`](AGENTS.md)。

契约修订：`doc-r1`（2026-08-21）
