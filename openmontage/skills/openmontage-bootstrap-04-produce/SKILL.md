...
| `ai_plan`（表 ③） | 按已确认分段的文案 / 提示词约束生成与拼接；不得擅自改段数或重写提示词而不再问用户。商品片：若 `gap_fill=i2i` 且补图未完成 → **先补图再 I2V**；某段无 `ref_image` 或缺口未关闭 → **不烧该段**，回 usercheck。重点段优先重试与打磨（见 `references/product-prompt-template.md`） |
