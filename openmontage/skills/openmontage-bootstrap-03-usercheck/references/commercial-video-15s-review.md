...
≥15s 时**默认普通**。专业模式仅当用户主动要求（「专业 / 逐段审」）。  
网页可标「可切专业」及差异；**聊天不主动推销专业**。  
普通模式下：完整总览、逐 beat 卡、候选状态机、五点抽帧等仍作为**内部证据**生成与保存，但不默认甩给用户。  
极简 = 库页/看板选「极简」时的打断策略：用户只确认方案、素材、交付三停。素材 `assets_gate=completed` 后交接 04 **直接生成正式第一段**，不做独立试片、不等待试片/初稿人审；只拦技术探测（文件可播、时长）、费用闸、缺 Key 与生成失败。普通仍含试片+初稿人审；专业七停都要过目。聊天里说「直接出片」仍走快速模式授权卡（与库页极简不是同一入口）。

### 0.2 画面构成比例（推荐目标 · 软约束）

简报字段：`motion_mix`（`0:1` / `1:2` / `1:1` / `2:1`）、`ai_share_pct`、`motion_mix_source`。

| 运镜:AI生成 | AI生成视频约占 | 说明 |
|-------------|---------------|------|
| **0:1** | **~100%** | **默认**；具体生成情况视模型能力而定 |
| 1:2 | ~70% | 可选；更动感 |
| 1:1 | ~50% | 可选，不作为推荐 |
| 2:1 | ~30% | 仅用户主动要更省时才提 |
...
03-usercheck：Grill 确认卡锁定主题、档位、时长、评审模式、渠模、motion_mix；
            ...
            再锁定 video_plan；默认普通评审，专业仅用户主动要。
