...
**0. 开口前先扫 Key（强制）**  
只扫三处：① 仓根 `.env` ② `.env-example` 中的视频渠道变量名 ③ 当前 Cursor/MCP 是否已注入同名 Key。不扫其它项目目录、不扫聊天。空 Key 禁止付费 generate。

- **有可用视频 Key** → 第一次即可走电商宣传片（渠模仍须用户确认后才 generate）。
...
- 用户说「生成视频」且环境已就绪、无锁定简报时：已按「就绪接话」**先扫 Key**，**默认电商宣传片**；已发完整 Backlot URL；未明确要求时**未**先推轻度 Demo  
