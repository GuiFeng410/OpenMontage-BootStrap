...
9. `verify_ready` — when `can_produce_video_now`（或等价 ready）为真即可交接下游。读返回的 `recommendations`：若缺 HF，仍可交接，但用一句话提醒「建议补装 HyperFrames，首次 Demo 可多一条品牌路径」。  
10. 交接：模糊需求 → **`openmontage-bootstrap-03-usercheck`**（推荐首次轻度短 Demo）；简报已锁定 → **`openmontage-bootstrap-04-produce`**。
...
- 向用户可加一句：「环境已就绪；旁白默认 Edge-TTS；建议有 Node 22+ 时再装 HyperFrames，也可先只跑 Remotion demo。」
